...
**特别注意**：
- 张量涉及多行定义时，矩阵每行末尾、独立一行均不能添加注释（即“[]”之间不能有注释）。
...
matrix = [[1, 2], [3, 4]];
```
**注意**：变量赋值只支持数值（包括复数，tensoreval运行时需加“--complex”）或者张量两种类型。
### 4.2 常量声明
```rustscript
// 常量定义（约定使用大写）
PI = 3.14159265359;
...
输入参数：
    ...
    step - 标量，步长（缺省时，默认步长为1）
    序列的开闭区间为左闭右开，即[start,end)
示例：
    输入 range(1, 5)
    结果 [1, 2, 3, 4]
...
输入参数：
    ...
    注：[start,end,step]中step可缺省（缺省时默认步长为1），左闭右开，即[start,end)
返回类型：
...
电力系统应用示例：
    bus_voltage = slice(bus, [0], [VM-1, VM]);    // 所有行，VM列(二维张量)
    bus_angle = slice(bus, [0], VA-1);            // 所有行，VA列(一维张量)
```
注：slice张量切片是连续或者按一定步长取张量的元素并组成新张量，若需取指定行列（离散）可使用select函数。
...
示例：
    ...
    输入 select(a, 0, [0,2])    //当行参数为单个数值、列参数为向量时，返回二维行向量。
    结果 [[1,3]]

    输入 select(a, [0,2], 0)    //当行参数为向量、列参数为单个数值时，返回二维列向量。
    结果 [[1], [7]]
```
...
输入参数：
    ...
    col_spec - 列索引，数值代表单个索引，[0]代表所有列，[start,end,step]代表范围和步长
    注：
    1）assign函数返回的是返回新张量，未修改原张量。
    2）[start,end,step]中step可缺省（缺省时默认步长为1），左闭右开，即[start,end)
    3）source的维度必须与target切片的形状匹配
    4）assign函数可以理解为target张量的slice切片部分替换为新的source张量
输入参数示例：
...
```
**注意**：循环序列需“0..10”格式，不能跟tensor；b为小数时，i取到小于b的最大整数为止。
...
示例：
    ...
    输入 svd([[1, 2, 3], [4, 5, 6]])
    结果 [9.508032000695724, 0.7728696356734843]
```
### 10.2.1.7 1范数
```rustscript
函数：
...
输入参数：
    t - 张量
    indices - 张量，其中元素表示t按行为主顺序排列的位置索引
    values - 张量，形状应与indices相同，indices中索引位置元素设置为values中的对应值
示例：
    输入 set([[1, 2], [3, 4]], [2, 0], -3)  //第3个和第1个元素赋值为-3
    结果 [[-3, 2], [-3, 4]]
...
#### 10.3.4 序列生成
#### 10.3.4.1 生成序列 [start, start+1, ..., end-1] (前闭后开)
```rustscript 
range(start, end)
```
#### 10.3.4.2 在start和end间生成num个等间距点
```rustscript
